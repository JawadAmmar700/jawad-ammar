...
const Details = ({ specificProject }: { specificProject: string }) => {
  ...
  return (
    <div className="w-full h-screen md:relative flex flex-col md:flex-none space-y-2 items-center justify-center">
      ...
      />
      <div className="w-full md:h-screen h-auto md:absolute md:top-0 flex items-center justify-center">
        <div className="w-full md:w-[600px] text-white flex flex-col space-y-3 justify-center z-30 opacity-100">
          <Link href="/#Showcase">
            <motion.div
              initial={{ scale: 0.9 }}
              whileHover={{ scale: 1 }}
          ...
          <p className="text-sm font-medium w-full text-center md:text-left break-words px-5">
            Libraries: {data.technology}
          </p>

          <p className="first-letter:text-xl md:first-letter:text-2xl text-sm px-5 font-medium text-center md:text-left md:h-[150px] h-[110px] break-words w-full overflow-y-scroll hide-scroll-bar cursor-all-scroll">
            {data.description}
          </p>
